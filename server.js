...
Globalize = {
  load: function(moduleName, className) {
    ...
    var globalize = Npm.require('globalize');
    var module = globalize.load(moduleName);

    if (!className)
      return module;

    // Get a reference to the original interface
    var interface = globalize.load(moduleName)[className];
    ...
    _.each(interface.prototype, function(fn, name) {
      ...
      // Make sure it's a method and it conforms to the globalize naming
      ...
      // globalize: https://github.com/appsattic/node-globalize/blob/master/lib/globalize.js#L325
